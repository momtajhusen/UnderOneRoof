...
const RegistrationOwnerScreen = ({ navigation, route }) => {
  ...
  const [ownerMobile, setOwnerMobile] = useState(''); // Will store only the 10-digit number
  ...
  const handleOwnerMobileChange = (text) => {
    ...
    if (digits.length > 10) {
      // If we already have 10 digits, ignore extra manual input.
  ...
  const validateForm = () => {
    ...
    if (!selectedBusinessType) {
      ...
      // Trigger shake animation for business type selection error
      Animated.sequence([
  ...
  useEffect(() => {
    if (fname.trim()) setFormErrors(prev => ({ ...prev, fname: '' }));
  }, [fname]);

  useEffect(() => {
    if (lname.trim()) setFormErrors(prev => ({ ...prev, lname: '' }));
  }, [lname]);

  useEffect(() => {
    if (email && /\S+@\S+\.\S+/.test(email)) setFormErrors(prev => ({ ...prev, email: '' }));
  }, [email]);

  useEffect(() => {
    if (ownerMobile && /^[6-9]\d{9}$/.test(ownerMobile)) setFormErrors(prev => ({ ...prev, ownerMobile: '' }));
  }, [ownerMobile]);
    ...
    try {
      ...
      if (response.data?.status === 1) {
        ...
        if (response.data.msg === "Email Already Registered") {
          setFormErrors(prev => ({ ...prev, email: response.data.msg }));
        } else {
        ...
        <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.checkboxContainer}>
            ...
            >
              <Animated.Text
                style={[
                  { fontWeight: '400', fontSize: rf(2), color: '#717171' },
...
const styles = StyleSheet.create({
  ...
  scrollContainer: { paddingBottom: rh(20) },
  informationCard: {
    height: rh(17),
    paddingTop: rh(1),
